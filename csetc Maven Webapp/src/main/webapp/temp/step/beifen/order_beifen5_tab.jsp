			...
			<div class="modal-content animated fadeIn">
				...
				<script>
					...
					function uploadCustomerQualification() {
						var orders = $('#orderTable').bootstrapTable(
								'getSelections');
						if (orders.length != 1) {
						...
						if (!(orders[0].status == 1013 || orders[0].status == 1012)) {
							toastr
									.warning("订单状态为\"已全款\"或者\"已收款,未全款\"的订单才允许上传客户资质");
							return false;
						}
				...
				<div class="modal-footer">
                    <button type="button" class="btn btn-white" data-dismiss="modal">关闭</button>
                    <button type="button" class="btn btn-primary" onclick="updBasecodeInfo()">上传</button>
                </div>
			</div>
		</div>
	</div>
