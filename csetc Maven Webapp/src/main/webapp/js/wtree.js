...
// Node object
function Node(id, pid, name, url, title, target, icon, iconOpen,checked, open) {
	this.id = id;
	this.pid = pid;
	this.name = name;
	this.url = url;
...
dTree.prototype.add = function(id, pid, name, url, title, target, icon, iconOpen, checked, open) {
	this.aNodes[this.aNodes.length] = new Node(id, pid, name, url, title, target, icon, iconOpen, checked, open);
};
...
dTree.prototype.node = function(node, nodeId) {
	...
	if(this.config.check==true){
		if(node.checked){
			str+= '<input type="checkbox" checked="checked" value="'+ node.id +'" class="cx" id="c'+  this.obj + nodeId + '" onclick="javascript:'+this.obj+'.cc('+nodeId+')"/>';
		}else{
			str+= '<input type="checkbox" value="'+ node.id +'" class="cx" id="c'+  this.obj + nodeId + '" onclick="javascript:'+this.obj+'.cc('+nodeId+')"/>';
		}
...
/**set default checked*/   
dTree.prototype.setDefaultCheck=function(nodeIds,treeName){  
    var ids = document.getElementsByClassName("cx");  
    var tempIds = nodeIds.split(",");     
    for(var n=0; n<tempIds.length; n++){ //loop default list  
        for(var i=0; i<ids.length; i++){  
        	//alert("c" + treeName + tempIds[n] +"\n"+ ids[i].value);        	
